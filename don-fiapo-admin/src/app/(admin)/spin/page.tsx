  ...
  const fetchSpinWallets = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const spinWs = (Array.isArray(all) ? all : [])
          .filter((w): w is WalletApiRow => Boolean(w && typeof w === "object" && "key" in w && "id" in w))
          .filter((w) => SPIN_WALLET_KEYS.includes(w.key))
          .map((w) => ({
            id: w.id,
            name: w.label,
            address: w.address,
              ...
              <div className="bg-neutral-900/40 border border-neutral-800/50 rounded-2xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {(stats.recentSpins ?? []).slice(0, 20).map((spin, i: number) => {
                        const playedAt = spin.playedAt ?? spin.createdAt;
                        const tier = spin.tier ?? "common";

                        return (
                          <tr key={i} className="border-b border-neutral-800/30 hover:bg-neutral-800/20">
                            <td className="px-6 py-3 text-sm text-neutral-400">
                              {playedAt ? new Date(playedAt).toLocaleString("pt-BR") : "-"}
                            </td>
                            ...
                            <td className="px-6 py-3">
                              <span className={cn("text-xs font-bold px-2 py-0.5 rounded-full border uppercase", TIER_COLORS[tier] || TIER_COLORS.common)}>
                                {tier}
                              </span>
                            </td>
                          </tr>
