...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetchWebApi(`/api/admin/wallet-txs?since=${encodeURIComponent(since)}&limit=1000`, {
      method: "GET",
    ...
    return NextResponse.json(
      { error: webApiProxyErrorMessage(err, "Failed to connect to don-fiapo-web"), detail: msg },
      { status: webApiProxyErrorStatus(err) }
    );
  }
