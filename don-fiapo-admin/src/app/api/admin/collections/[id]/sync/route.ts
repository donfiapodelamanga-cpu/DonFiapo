...
interface CollectionItemForSync {
  id: string;
  name: string;
...
export async function POST(
  ...
  try {
    ...
    const pendingItems = (collection.items as CollectionItemForSync[]).filter((item) => !item.onChainTokenId);

    for (const item of pendingItems) {
      const metadataUri = item.metadataIpfsHash
        ? `ipfs://${item.metadataIpfsHash}`
        : item.ipfsHash
          ? `ipfs://${item.ipfsHash}`
          : item.imageUrl || "";
    ...
    return NextResponse.json(
      { error: `Erro na sincronização: ${getErrorMessage(error)}` },
      { status: 500 }
    );
  }
}
