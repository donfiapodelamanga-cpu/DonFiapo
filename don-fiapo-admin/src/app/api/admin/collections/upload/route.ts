...
type ReadableWithPath = Readable & {
  path?: string;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadStream = stream as ReadableWithPath;
    uploadStream.path = pinataName;

    const result = await pinata.pinFileToIPFS(uploadStream, {
    ...
    const msg = errorMessage.includes("PINATA") || errorMessage.includes("obrigatórios")
      ? "PINATA_API_KEY e PINATA_SECRET_KEY não configurados no .env. Configure para habilitar upload IPFS."
      : `Falha no upload para IPFS: ${errorMessage}`;
