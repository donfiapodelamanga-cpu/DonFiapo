...
type BalanceFieldLike = {
  toString: () => string;
};

type AccountInfoLike = {
  data: {
    free: BalanceFieldLike;
    reserved: BalanceFieldLike;
  };
};

type ContractsApiCallResult = {
  result?: {
    isOk?: boolean;
    asOk?: {
      data: {
...
export async function getLunesBalance(address: string): Promise<LunesBalance> {
  ...
  const { data } = await api.query.system.account(address) as unknown as AccountInfoLike;
...
export async function getFiapoBalance(
  ...
  try {
    ...
    const result = await api.call.contractsApi.call(
      ...
      callData.toHex()
    ) as unknown as ContractsApiCallResult;
