...
function getContract(api: ApiPromise): ContractPromise {
  return new ContractPromise(api, ICO_CONTRACT_ABI as ContractMetadata, ICO_CONTRACT_ADDRESS);
}

// Helper to decode contract query result
function decodeQueryResult(result: QueryResultLike | null | undefined): unknown {
  const output = result?.output;
  if (result?.result?.isOk && output) {
    try {
...
export async function getICOStats(): Promise<ICOStats | null> {
  try {
    ...
    const statsData = toRecord(data);
    if (!statsData) return null;

    // data is ICOStats struct
    const totalRaisedCents = parseNumber(statsData.totalRaisedUsdtCents ?? statsData.total_raised_usdt_cents ?? 0);
    const totalClaimed = parseBigNumber(statsData.totalTokensClaimed ?? statsData.total_tokens_claimed ?? "0");

    return {
      totalNftsMinted: parseNumber(statsData.totalNftsMinted ?? statsData.total_nfts_minted ?? 0),
      totalRaisedUsdCents: totalRaisedCents,
      totalRaisedUsd: totalRaisedCents / 100,
      totalTokensMined: parseBigNumber(statsData.totalTokensMined ?? statsData.total_tokens_mined ?? "0") / SCALE,
      totalTokensClaimed: totalClaimed / SCALE,
      uniqueParticipants: parseNumber(statsData.uniqueParticipants ?? statsData.unique_participants ?? 0),
      icoActive: Boolean(statsData.icoActive ?? statsData.ico_active ?? false),
      miningActive: Boolean(statsData.miningActive ?? statsData.mining_active ?? false),
    };
  } catch (error) {
...
export async function getTierConfigs(): Promise<TierConfig[]> {
  try {
    ...
    return data.map((config, index: number) => {
      const configData = toRecord(config) ?? {};
      const minted = parseNumber(configData.minted ?? 0);
      const maxSupply = parseNumber(configData.maxSupply ?? configData.max_supply ?? 0);

      return {
        tier: index,
        name: TIER_NAMES[index] || `Tier ${index}`,
        priceUsdCents: parseNumber(configData.priceUsdtCents ?? configData.price_usdt_cents ?? 0),
        priceUsd: TIER_PRICES_USD[index] || 0,
        maxSupply,
        minted,
        mintedEvolution: parseNumber(configData.mintedEvolution ?? configData.minted_evolution ?? 0),
        burned: parseNumber(configData.burned ?? 0),
        tokensPerNft: parseBigNumber(configData.tokensPerNft ?? configData.tokens_per_nft ?? "0") / SCALE,
        dailyMiningRate: parseBigNumber(configData.dailyMiningRate ?? configData.daily_mining_rate ?? "0") / SCALE,
        active: Boolean(configData.active ?? true),
...
export async function getEvolutionStats(): Promise<EvolutionStats | null> {
  try {
    ...
    const evolutionData = toRecord(data);
    if (!evolutionData) return null;

    return {
      totalEvolutions: parseNumber(evolutionData.totalEvolutions ?? evolutionData["0"] ?? 0),
      totalNftsBurned: parseNumber(evolutionData.totalNftsBurned ?? evolutionData["1"] ?? 0),
    };
  } catch (error) {
  ...
  try {
    ...
    return data.map((item, index: number) => {
      ...
      const count = Array.isArray(item)
        ? parseNumber(item[1] ?? 0)
        : parseNumber(itemData?.count ?? itemData?.["1"] ?? 0);
      return {
