...
function getContract(api: ApiPromise): ContractPromise {
  return new ContractPromise(api, STAKING_CONTRACT_ABI as ContractMetadata, STAKING_CONTRACT_ADDRESS);
}
...
export async function getStakingStats(): Promise<StakingStats | null> {
  try {
    ...
    const totalStaked = parseBigNumber(statsData.totalStaked ?? statsData.total_staked ?? "0");
    const totalRewards = parseBigNumber(statsData.totalRewardsDistributed ?? statsData.total_rewards_distributed ?? "0");

    return {
      totalStaked: totalStaked / SCALE,
      totalStakedFormatted: formatTokens(totalStaked),
      totalStakers: parseNumber(statsData.totalStakers ?? statsData.total_stakers ?? 0),
      totalRewardsDistributed: totalRewards / SCALE,
      totalRewardsFormatted: formatTokens(totalRewards),
      activePositions: parseNumber(statsData.activePositions ?? statsData.active_positions ?? 0),
    };
  } catch (error) {
