...
export const SESSION_COOKIE = "admin_session_token";
...
function sign(payloadB64: string): string | null {
...
export function requireAdminAuth(req: NextRequest, requiredPermission?: string) {
  if (!isAdminSessionConfigured()) {
    return {
      ok: false as const,
      response: getAuthErrorResponse("Admin auth is not configured", 500),
    };
  }
