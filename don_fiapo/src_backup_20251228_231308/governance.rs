...
pub struct Governance {
    ...
    /// Lista de governadores (AccountId -> active: bool)
    governors: Vec<(AccountId, bool)>,
    /// Propostas por ID (limitado a 200)
    proposals: Vec<Proposal>,
    /// Votos por proposta e governador (proposal_id, voter, vote)
    votes: Vec<(u64, AccountId, WeightedVote)>,
    /// Pagamentos de governança (limitado a 500)
    ...
    /// Distribuições de taxas de governança (limitado a 200)
    fee_distributions: Vec<GovernanceFeeDistribution>,
    ...
    /// Remunerações de governadores (AccountId, GovernorRemuneration)
    governor_remunerations: Vec<(AccountId, GovernorRemuneration)>,
    /// Distribuições comunitárias (limitado a 100)
    community_distributions: Vec<CommunityDistribution>,
...
impl Encode for Governance {
    fn encode(&self) -> Vec<u8> {
        let mut result = self.config.encode();
        result.extend(self.next_proposal_id.encode());
        result.extend(self.next_payment_id.encode());
        result.extend(self.total_governors.encode());
        result.extend(self.is_active.encode());
        result.extend(self.staking_contract.encode());
        result.extend(self.next_distribution_id.encode());
        result.extend(self.total_fees_collected.encode());
        result.extend(self.total_staking_distributed.encode());
        result.extend(self.total_rewards_distributed.encode());
        result.extend(self.total_team_distributed.encode());
        result.extend(self.next_community_distribution_id.encode());
        result.extend(self.last_community_distribution.encode());
        result.extend(self.remuneration_config.encode());
        result
    }
}

impl Decode for Governance {
    fn decode<I: scale::Input>(input: &mut I) -> Result<Self, scale::Error> {
        let config = GovernanceConfig::decode(input)?;
        let next_proposal_id = u64::decode(input)?;
        let next_payment_id = u64::decode(input)?;
        let total_governors = u32::decode(input)?;
        let is_active = bool::decode(input)?;
        let staking_contract = Option::<AccountId>::decode(input)?;
        let next_distribution_id = u64::decode(input)?;
        let total_fees_collected = u128::decode(input)?;
        let total_staking_distributed = u128::decode(input)?;
        let total_rewards_distributed = u128::decode(input)?;
        let total_team_distributed = u128::decode(input)?;
        let next_community_distribution_id = u32::decode(input)?;
        let last_community_distribution = u64::decode(input)?;
        let remuneration_config = RemunerationConfig::decode(input)?;

        Ok(Self {
            config,
            governors: Mapping::default(),
            ...
            total_staking_distributed,
            total_rewards_distributed,
            total_team_distributed,
            governor_remunerations: Mapping::default(),
            community_distributions: Mapping::default(),
            next_community_distribution_id,
            last_community_distribution,
            remuneration_config,
        })
    }
}
...
impl Governance {
    ...
    pub fn calculate_governor_remunerations(&mut self, total_amount: Balance) -> Result<(), GovernanceError> {
        ...
        // Conta governadores ativos
        let active_governors = self.total_governors;
        let governor_list: Vec<AccountId> = Vec::new(); // Placeholder

        
        // Alternativa: A lista pode ser passada como parâmetro da função
        // ou mantida como Vec separado no storage do contrato
        
        // Distribui igualmente entre governadores ativos (se a lista estiver populada)
        if active_governors > 0 && !governor_list.is_empty() {
            let share_per_governor = governor_share.saturating_div(active_governors.into());
            
            for governor in governor_list {
                let mut remuneration = self.governor_remunerations.get(&governor).unwrap_or_default();
                remuneration.accumulated_amount = remuneration.accumulated_amount.saturating_add(share_per_governor);
                self.governor_remunerations.insert(&governor, &remuneration);
            }
        }
